package nl.toonbrand.projecteuler;

/**
 * Created by toon on 8/9/16.
 */
public class ProjectEuler8 {
    public static void main(String[] args) {
        final long startTime = System.currentTimeMillis();
        String bigNumber="";

        System.out.println("Largest product: " + findLargestProduct(bigNumber));

        final long endTime = System.currentTimeMillis();
        System.out.println("\n[Execution time: " + (endTime - startTime) + " milliseconds]");



    }
    public static long findLargestProduct(String bigNumber){
        int numberLength=bigNumber.length();
        long largestNumber=0;

        for(int i=0;i<numberLength-12;i++){
            String numberRow = bigNumber.substring(i, i+13);

            long rowProduct=calculateRowProduct(numberRow);

            if(rowProduct>largestNumber){
                largestNumber=rowProduct;
            }
            System.out.println("The product of " + numberRow + " = " + rowProduct);
        }
        return largestNumber;
    }

    public static long calculateRowProduct(String input){
        long product = 1L;
        for(int i=0;i<input.length();i++){
            product = product * Integer.parseInt(input.substring(i,i+1));
        }
        return product;
    }
}
